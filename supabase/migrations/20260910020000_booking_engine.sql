alter type public.appointment_status add value if not exists 'scheduled' after 'pending';
...
alter table public.tenant_settings
  ...
  add column no_show_policy public.refund_policy not null default 'client_credit';

alter table public.appointments
  add column vip_discount_amount numeric(12,2) not null default 0 check (vip_discount_amount >= 0),
  add column fidelity_discount_amount numeric(12,2) not null default 0 check (fidelity_discount_amount >= 0),
  add column fidelity_card_id uuid references public.fidelity_cards(id) on delete set null,
  add column cancelled_by public.user_role;
...
create index refund_requests_tenant_status_idx
on public.refund_requests (tenant_id, status, created_at);

create unique index commissions_no_show_once_idx
on public.commissions (appointment_id)
where is_no_show;

create unique index client_credits_cancellation_once_idx
